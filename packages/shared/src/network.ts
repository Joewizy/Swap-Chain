 ...
 *
 * Flip the network flag between `testnet` and `mainnet` and every rail
 * (RainbowKit / Reown, Relay quote API, Chainrails, CCTP, Paycrest, the intent
 * parser, the route planner) reads from this file. The web app sets it via
 * `NEXT_PUBLIC_NETWORK`; the mobile app via `EXPO_PUBLIC_NETWORK`. Each bundler
 * statically inlines only its own prefix, so the other reads as `undefined`.
 *
...
export const NETWORK: NetworkMode =
  ((process.env.NEXT_PUBLIC_NETWORK ??
    process.env.EXPO_PUBLIC_NETWORK) as NetworkMode) || "testnet";
