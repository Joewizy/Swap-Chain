...
import { RelayExecutionResponse } from "@/types";
import { ACTIVE_CHAINS, ACTIVE_TOKENS, IS_MAINNET, NETWORK, getTokenAddress } from "@/config/network";
...
export default function Transfer() {
  ...
  const defaultSourceChain =
    ACTIVE_CHAINS.find((c) => c.kind === "evm")?.id || "";
  const defaultTargetChain =
    ACTIVE_CHAINS.find((c) => c.kind === "evm" && c.id !== defaultSourceChain)
      ?.id || defaultSourceChain;
  const [sellToken, setSellToken] = useState(IS_MAINNET ? "USDC" : "ETH");
  const [buyToken, setBuyToken] = useState(IS_MAINNET ? "USDC" : "ETH");
  ...
  // Starknet state
  const [starkSellToken, setStarkSellToken] = useState<
    "STRK" | "ETH" | "USDC" | "USDT"
  >("STRK");
  const [starkBuyToken, setStarkBuyToken] = useState<
    "STRK" | "ETH" | "USDC" | "USDT"
  >("STRK");
  ...
  const {
    ...
    error: starkConnectError,
    isPending: starkIsPending,
  } = useStarknetConnect() as any;
  const { disconnect: disconnectStarknet } = useStarknetDisconnect();

  const wrapperRef = useRef<HTMLDivElement>(null);

  // Configuration follows NEXT_PUBLIC_NETWORK through src/config/network.ts.
  const availableChains = ACTIVE_CHAINS.filter((c) => c.kind !== "starknet");
  const getChainById = (id: string | null | undefined) =>
    availableChains.find((c) => c.id === (id || ""));
  const sourceChainCfg = getChainById(sourceChain);
  const targetChainCfg = getChainById(targetChain);
  const availableSourceTokens = ACTIVE_TOKENS.filter(
    (t) => sourceChainCfg && getTokenAddress(t.symbol, sourceChainCfg.id)
  );
  const availableTargetTokens = ACTIVE_TOKENS.filter(
    (t) => targetChainCfg && getTokenAddress(t.symbol, targetChainCfg.id)
  );
  ...
  useEffect(() => {
    if (
      sourceChainCfg &&
      !availableSourceTokens.find((t) => t.symbol === sellToken)
    ) {
  ...
  useEffect(() => {
    if (
      targetChainCfg &&
      !availableTargetTokens.find((t) => t.symbol === buyToken)
    ) {
  ...
  const getQuote = async () => {
    ...
    const isTargetSolana =
      targetChain === "solana" || targetChain === "solana-devnet";
    if (isTargetSolana) {
      if (!recipient) {
    ...
    try {
      ...
      if (quoteData.success) {
        setQuote(quoteData);
        setBuyAmount(
          quoteData.quote?.details?.currencyOut?.amountFormatted ||
            quoteData.amount
        );
        setQuoteError("");
        setAiResponse(
          `Got your quote! Route: ${quoteData.fromChain} → ${quoteData.toChain}.`
        );
      } else {
  ...
  async function executeQuote(quote: RelayExecutionResponse): Promise<{
    success: boolean;
    txHash?: string;
    txLink?: string;
    error?: string;
  ...
  const isOnCorrectNetwork = () => {
    ...
    const targetChainId = sourceChainCfg?.numericId;
  ...
  const handleExecute = async () => {
    ...
    if (!isOnCorrectNetwork()) {
      setAiResponse(
        `Please switch your wallet to ${sourceChainCfg?.name || "the correct network"} before executing. Current network: ${chain?.name || "Unknown"}`
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setAiResponse(
          "Transaction executed successfully! Check the transaction details below."
        );
        ...
        setAiResponse(
          `Sorry, I encountered an error executing your transaction: ${errorMessage}`
        );
      }
    } catch (error) {
      ...
      setAiResponse(
        `Sorry, I encountered an error executing your transaction: ${message}`
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAiIntent = async () => {
    ...
    try {
      ...
      if (result.needsClarification || result.action === "unclear") {
        setAiResponse(
          result.clarificationQuestion ||
            "I need a source chain, destination chain, token, and amount before I can fill the form."
        );
        return;
      }

      if (result.fromToken) setSellToken(result.fromToken);
      if (result.toToken || result.fromToken) {
        setBuyToken(result.toToken || result.fromToken);
      }
      if (result.fromAmount) setSellAmount(String(result.fromAmount));
      if (result.fromChain) {
        if (!availableChains.find((c) => c.id === result.fromChain)) {
          setAiResponse(
            `I parsed your intent, but "${result.fromChain}" is not available in ${NETWORK} mode.`
          );
          return;
        }
        setSourceChain(result.fromChain);
      }
      if (result.toChain) {
        if (!availableChains.find((c) => c.id === result.toChain)) {
          setAiResponse(
            `I parsed your intent, but "${result.toChain}" is not available in ${NETWORK} mode.`
          );
          return;
        }
        setTargetChain(result.toChain);
      }

      const actionLabel =
        result.action === "offramp"
          ? "I parsed this as a payout/off-ramp request. Payout routing is not live yet, so I filled the crypto fields that are available."
          : `I parsed your ${result.action} request and filled the form for review.`;
      setAiResponse(actionLabel);
                ...
                {starkStatus === "connected" && starkAddress ? (
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                    ...
                    <div className="space-y-2">
                      {Array.isArray(starkConnectors) &&
                      starkConnectors.length > 0 ? (
              ...
              {starkConnectError && (
                <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <p className="text-xs text-red-600 mt-1">
                    {String(
                      (starkConnectError as Error).message || starkConnectError
                    )}
                  </p>
                </div>
              )}
      ...
      <div className="flex flex-col items-center w-full max-w-md mx-auto mb-44">
        <div className="flex mb-4 justify-between w-full px-2">
          <div>
            <div className="flex items-center gap-3">
              <span className="text-sm rounded-full py-1 px-3 bg-primary-20 text-primary">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-1 border border-primary-30 rounded-full py-1 px-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setStarkSellToken(
                        e.target.value as "STRK" | "ETH" | "USDC" | "USDT"
                      )
                    }
                  >
          ...
          {!quote ? (
            ...
            <div>
              ...
              <div className="flex justify-between mt-2">
                ...
                <span className="text-xs">
                  ${Number(quote.quote?.fees?.gas?.amountUsd || 0).toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between mt-2">
                ...
                <span className="text-xs text-red-500">
                  ...
                  {(() => {
                    ...
                    const relayer = Number(
                      quote.quote?.fees?.relayerService?.amountUsd || 0
                    );
              ...
              <div className="flex justify-between mt-2">
                ...
                <span className="text-xs">
                  {quote.quote?.details?.timeEstimate
                    ? `${quote.quote.details.timeEstimate} min`
                    : "—"}
                </span>
              </div>
            </div>
        ...
        {swapDomain === "starknet" && (
          <button
            ...
            onClick={() =>
              setAiResponse(
                "Starknet swaps are paused while client-side signing is rebuilt."
              )
            }
            disabled
          >
            Starknet Swaps Paused
          </button>
        )}
      </div>
